...
performCookieAuth(boost::beast::http::verb method [[maybe_unused]],
  ...
  std::shared_ptr<persistent_data::UserSession> sessionOut =
      persistent_data::SessionStore::getInstance().loginSessionByToken(authKey);
...
static std::shared_ptr<persistent_data::UserSession>
performTLSAuth(Response &res, const boost::beast::http::header<true> &reqHeader,
  ...
  if (auto sp = session.lock()) {
    ...
    res.addHeader(boost::beast::http::field::set_cookie,
                  "XSRF-TOKEN=" + sp->csrfToken + "; SameSite=Strict; Secure");
    res.addHeader(boost::beast::http::field::set_cookie,
